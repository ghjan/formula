	...
	for _, tt := range tests {
		t.Run(fmt.Sprint(tt.arg0, tt.arg1), func(t *testing.T) {
			...
			if utils.IsNum(tt.arg0) && utils.IsNum(tt.arg1) {
				got, err = g.Evaluate(nil, []*opt.LogicalExpression{exp.NewFloatExpression(tt.arg0),
					exp.NewFloatExpression(tt.arg1)}...)
			} else {
				...
				if (val1 == "true" || val1 == "false") || (val2 == "true" || val2 == "false") {
					...
					got, err = g.Evaluate(nil, []*opt.LogicalExpression{exp.NewBooleanValueExpression(val1Bool),
						exp.NewBooleanValueExpression(val2Bool)}...)
				} else {
					got, err = g.Evaluate(nil, []*opt.LogicalExpression{exp.NewStringValueExpression(tt.arg0),
						exp.NewStringValueExpression(tt.arg1)}...)
				}
			}
			if err != nil {
				t.Fatal(err)
...
func TestNotGreaterFunction_Evaluate(t *testing.T) {
	tests := []struct {
		arg0 string
		arg1 string
		want bool
...
func TestNotLessFunction_Evaluate(t *testing.T) {
	tests := []struct {
		arg0 string
		arg1 string
		want bool
